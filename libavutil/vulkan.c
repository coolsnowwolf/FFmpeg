...
void ff_vk_exec_pool_free(FFVulkanContext *s, FFVkExecPool *pool)
{
    ...
    for (int i = 0; i < pool->pool_size; i++) {
        if (pool->cmd_buf_pools && pool->cmd_buf_pools[i])
...
FFVkExecContext *ff_vk_exec_get(FFVulkanContext *s, FFVkExecPool *pool)
{
    ...
    (void)s;
    idx = atomic_fetch_add(&pool->idx, 1);
    idx %= pool->pool_size;
                                ...
                                VkMemoryHostPointerPropertiesEXT props)
{
    ...
    err = ff_vk_alloc_mem(s, &req, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
                          ...
                          VkBufferUsageFlags usage)
{
    ...
    *dst = NULL;
    ...
    err = create_mapped_buffer(s, vkb, usage, buffer_size,
                               &create_desc, &import_desc, props);
    if (err < 0) {
        av_buffer_unref(&ref);
        av_free(vkb);
        return err;
    ...
    *dst = av_buffer_create((uint8_t *)vkb, sizeof(*vkb), destroy_avvkbuf, s, 0);
    if (!(*dst)) {
        destroy_avvkbuf(s, (uint8_t *)vkb);
        return AVERROR(ENOMEM);
    }
