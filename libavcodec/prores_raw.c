                       ...
                       AVFrame *frame)
{
    ...
    header_len = bytestream2_get_byteu(gb) >> 3;
    ...
    comp_start = gb->buffer_start + header_len;
                        ...
                        AVPacket *avpkt)
{
    ...
    header_len = bytestream2_get_be16(&gb);
    if (header_len < 62)
        return AVERROR_INVALIDDATA;
    ...
    w = bytestream2_get_be16(&gb_hdr);
    h = bytestream2_get_be16(&gb_hdr);
    if ((w & 1) || (h & 1))
        return AVERROR_INVALIDDATA;
    ...
    flags = bytestream2_get_be16(&gb_hdr);
    align = (flags >> 1) & 0x7;
    ...
    if (avctx->hwaccel) {
        ...
        ff_refstruct_unref(&s->hwaccel_picture_private);
    } else {
